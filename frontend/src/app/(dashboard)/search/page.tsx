...
function SearchPageContent() {
  ...
  const handleAsk = useCallback(() => {
    ...
    // Use Direct AI mode (no RAG)
    if (useDirectAI) {
      ask.sendDirectAsk(askQuestion, modelDefaults?.default_chat_model || undefined)
      return
    }

    // Use RAG mode (requires chat model)
  ...
  }, [askQuestion, modelDefaults, customModels, ask, useDirectAI])
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  {useDirectAI
                    ? 'Ask AI directly using its general knowledge (faster)'
                    : 'Ask AI using your knowledge base documents (RAG mode)'
                  }
                </p>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {useDirectAI ? 'Direct AI (General Knowledge)' : 'RAG (Your Documents)'}
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs ${!useDirectAI ? 'text-primary font-medium' : 'text-muted-foreground'}`}>RAG</span>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setUseDirectAI(!useDirectAI)}
                      className="relative w-12 h-6 p-0"
                    >
                      <div className={`absolute w-5 h-5 rounded bg-primary transition-all ${useDirectAI ? 'translate-x-3' : '-translate-x-2'}`} />
                    </Button>
                    <span className={`text-xs ${useDirectAI ? 'text-primary font-medium' : 'text-muted-foreground'}`}>Direct</span>
                  </div>
                </div>
                {/* Question Input */}
                <div className="space-y-2">
                ...
                {/* Notebook Filter */}
                <div className="space-y-2">
                  <Label htmlFor="notebook-filter" className="flex items-center gap-2">
                  ...
                  {selectedNotebook !== 'all' && (
                    <p className="text-xs text-muted-foreground">
                      Searching only in: {notebooks.find(n => n.id === selectedNotebook)?.name}
                    </p>
                  )}
                </div>
